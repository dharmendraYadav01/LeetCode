...
public:
    int helper(int m,int n,string& word1,string& word2,vector<vector<int>>&vec){
        ...
        if(word1[m-1]==word2[n-1]){
            return vec[m][n]=helper(m-1,n-1,word1,word2,vec);
        }else{
            return vec[m][n]=min(min(1+helper(m,n-1,word1,word2,vec),1+helper(m-1,n-1,word1,word2,vec)),1+helper(m-1,n,word1,word2,vec));
        }
    }
